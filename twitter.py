...
def twitter(term,sort):
  #These keys are critical for use of our Twitter API. They are converted to base64 so that they can be utilized properly by Twitter
  ...
  #Our base URL is the root that all requests are made to. The auth url is the utilization of our secure application OATH token.
  ...
  #Search parameters (search term [string], result_type [string - mixed, recent, popular], and count. Maximum allowed is 100 results. )
  search_params = {
    'q': term,
    'result_type': sort,
    'count': 100
  ...
  #This section makes a web request similar to above, but will let us know on the backend how many more requests we are allowed to make within a 15 minute period. Twitter limits us to 450 requests per 15 minutes.
  ...
  #This is the output of how many quests we have left. Not necessary to keep, but good info to have for debugging purposes.
